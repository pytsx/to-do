...
function Pytsx({ sup, disableIcon, disableName }: { sup?: string, disableName?: boolean, disableIcon?: boolean }) {
  return (
    <Link aria-label="redirecionar para o criador do site" href={""}>
      <div className="flex flex-row gap-3 select-none mx-[2px]">
        {!disableIcon && (<span className="p-2 border border-neutral-800 rounded-md w-9 h-9 flex items-center justify-center">
          <svg xmlns="http://www.w3.org/2000/svg"
            aria-label={`${process.env.SITE_NAME} logo`}
            viewBox="0 0 93 93"
            className={" h-4 w-4 fill-neutral-950 dark:fill-neutral-400"}
          >
        ...
        {!disableName && (
          <h1 className="my-auto uppercase text-md font-semibold ml-2 text-neutral-300">to do</h1>
        )}
        {sup && (
